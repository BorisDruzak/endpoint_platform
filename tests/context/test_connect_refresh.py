...
async def test_connect_refresh_queues_only_supported_inventory_profiles(session) -> None:
    ...
    assert created == 2
    assert [collection.profile for collection in queued] == ["baseline_v1", "inventory_v1"]
...
async def test_connect_refresh_does_not_duplicate_active_collection(session) -> None:
    ...
    session.add(device)
    await session.flush()
    capabilities = {"context.baseline.collect", "context.inventory.collect"}
    now = datetime(2026, 9, 18, 10, 0, tzinfo=UTC)

    assert await queue_connect_refreshes(session, device.id, capabilities, now=now) == 2
