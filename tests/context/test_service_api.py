...
def _principal(
    scopes: list[str], *, client_identifier: str = "web-ovpn"
) -> ServicePrincipal:
...
def _normalized_projection(profile: str, collected_at: datetime) -> dict[str, object]:
    sections_by_profile: dict[str, dict[str, object]] = {
        "baseline_v1": {
            ...
            "storage": [
                {"stable_key": "disk:one", "model": "Disk", "size_bytes": 2048}
            ],
            "interfaces": [],
            "software": [],
        },
        "health_v1": {
            "resources": {"uptime_seconds": 1, "load_1m": 0.0, "free_bytes": 1},
            "services": [],
        },
        "network_v1": {
            "default_route": {"interface": "eth0", "gateway": "192.0.2.1"},
            "interfaces": [],
        },
    }
    return {
        ...
        "warnings": [],
        "sections": sections_by_profile[profile],
    }


@pytest.mark.asyncio
async def test_service_context_read_projection_excludes_raw_payload_and_diagnostics(
    ...
    async with session_provider() as session:
        ...
        device = Device(
            id=uuid4(),
            device_identifier="context-device",
            display_name="Context device",
            retired_at=None,
        )
        collection = ContextCollection(
            ...
            raw_result_payload={"token": "secret"},
        )
        snapshot = ContextSnapshot(
            ...
            semantic_hash="b" * 64,
            raw_payload={"result_items": [{"traceback": "raw diagnostic"}]},
            normalized_projection={
                "schema_version": "device_context_v1",
                "profile": "baseline_v1",
                "warnings": [],
                "sections": {"system": {"platform": "linux"}},
            },
        )
    ...
    async with httpx.AsyncClient(
        ...
        response = await client.get(
            f"/api/v1/devices/{device.id}/context",
...
@pytest.mark.asyncio
async def test_safe_context_routes_exclude_and_reject_diagnostic_collections(
    session_provider: async_sessionmaker[AsyncSession], monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    async with session_provider() as session:
        device = Device(
            id=uuid4(),
            device_identifier="diagnostic-device",
            display_name="Diagnostic device",
            retired_at=None,
        )
        diagnostic = ContextCollection(
            ...
            idempotency_key="diagnostic-seed",
            status="completed",
            requested_at=datetime.now(UTC),
    ...
    _install_principals(
        monkeypatch,
        {
            "reader": _principal(["context.read"]),
            "collector": _principal(["context.collect"]),
        },
    )
    app = create_app(_settings(), session_provider)
    async with httpx.AsyncClient(
        ...
        read_diagnostic = await client.get(
            f"/api/v1/context/collections/{diagnostic.id}",
            headers={"Authorization": "Bearer reader"},
        )
        request_diagnostic = await client.post(
            f"/api/v1/devices/{device.id}/context/collections",
            json={"profile": "diagnostic_v1"},
            headers={
                "Authorization": "Bearer collector",
                "Idempotency-Key": "diagnostic-request",
            },
        )
    assert listed.status_code == 200
    assert listed.json()["data"]["profiles"] == [
        {"profile": "baseline_v1", "status": "completed", "last_collected_at": None}
    ]
...
@pytest.mark.asyncio
async def test_device_listing_requires_devices_read_not_context_read(
    session_provider: async_sessionmaker[AsyncSession], monkeypatch: pytest.MonkeyPatch
) -> None:
    async with session_provider() as session:
        device = Device(
    ...
    async with httpx.AsyncClient(
        ...
        allowed = await client.get(
            "/api/v1/devices", headers={"Authorization": "Bearer device-reader"}
        )
        denied = await client.get(
            "/api/v1/devices", headers={"Authorization": "Bearer context-reader"}
        )
...
@pytest.mark.asyncio
async def test_collection_request_requires_exact_scope_idempotency_and_audit(
    session_provider: async_sessionmaker[AsyncSession], monkeypatch: pytest.MonkeyPatch
) -> None:
    async with session_provider() as session:
        ...
        device = Device(
            id=uuid4(),
            device_identifier="collect-device",
            display_name="Collect device",
            retired_at=None,
        )
        session.add(device)
        await session.commit()
    ...
    async with httpx.AsyncClient(
        ...
        denied = await client.post(
            ...
            headers={
                "Authorization": f"Bearer {denied_token}",
        ...
        created = await client.post(
            ...
            headers={
                ...
                "Idempotency-Key": "request-001",
                "X-Request-ID": "Bearer raw secret",
            },
        )
...
@pytest.mark.asyncio
async def test_collection_idempotency_is_scoped_to_the_authenticated_requester(
    session_provider: async_sessionmaker[AsyncSession], monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    async with session_provider() as session:
        device = Device(
            id=uuid4(),
            device_identifier="principal-device",
            display_name="Principal device",
            retired_at=None,
        )
        session.add(device)
        await session.commit()
    ...
    async with httpx.AsyncClient(
        ...
        first = await client.post(
            f"/api/v1/devices/{device.id}/context/collections",
            json={"profile": "baseline_v1"},
            headers={**headers, "Authorization": "Bearer first"},
        )
        second = await client.post(
            f"/api/v1/devices/{device.id}/context/collections",
            json={"profile": "baseline_v1"},
            headers={**headers, "Authorization": "Bearer second"},
        )
        replay = await client.post(
            f"/api/v1/devices/{device.id}/context/collections",
            json={"profile": "baseline_v1"},
...
@pytest.mark.asyncio
async def test_safe_context_snapshots_are_ordered_by_profile(
    session_provider: async_sessionmaker[AsyncSession], monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    async with session_provider() as session:
        device = Device(
            id=uuid4(),
            device_identifier="ordered-device",
            display_name="Ordered device",
            retired_at=None,
        )
        collections = [
            ContextCollection(
        ...
        snapshots = [
            ContextSnapshot(
                ...
                semantic_hash=None,
                raw_payload={},
                normalized_projection=_normalized_projection(
                    collection.profile, collected_at
                ),
            )
    ...
    assert response.status_code == 200
    assert [item["profile"] for item in response.json()["data"]["snapshots"]] == [
        "baseline_v1",
        "health_v1",
        "network_v1",
    ]
...
async def test_baseline_history_is_scoped_bounded_ordered_and_safe(
    ...
    async with session_provider() as session:
        device = Device(
            id=uuid4(),
            device_identifier="history-device",
        ...
        tied_first = ContextCollection(
            id=uuid4(),
            device_id=device.id,
            profile="baseline_v1",
            requested_by="svc",
            idempotency_key="tied-first",
            status="completed",
            requested_at=now,
        )
        tied_second = ContextCollection(
            id=uuid4(),
            device_id=device.id,
            profile="baseline_v1",
            requested_by="svc",
            idempotency_key="tied-second",
            status="completed",
            requested_at=now,
        )
        diagnostic = ContextCollection(
        ...
        snapshots = (
            ContextSnapshot(
                id=uuid4(),
                collection_id=oldest.id,
                device_id=device.id,
                profile="baseline_v1",
                collected_at=now.replace(year=2025),
                semantic_hash="a" * 64,
                raw_payload={"raw": "secret"},
                normalized_projection=_normalized_projection(
                    "baseline_v1", now.replace(year=2025)
                ),
            ),
            ContextSnapshot(
                id=ids[0],
                collection_id=tied_first.id,
                device_id=device.id,
                profile="baseline_v1",
                collected_at=now,
                semantic_hash="b" * 64,
                raw_payload={"raw": "secret"},
                normalized_projection=_normalized_projection("baseline_v1", now),
            ),
            ContextSnapshot(
                id=ids[1],
                collection_id=tied_second.id,
                device_id=device.id,
                profile="baseline_v1",
                collected_at=now,
                semantic_hash="c" * 64,
            ...
            ContextSnapshot(
                ...
                collected_at=now,
                semantic_hash=None,
                raw_payload={"raw": "secret diagnostic"},
                normalized_projection={},
            ),
            ContextSnapshot(
                id=uuid4(),
                ...
                collected_at=now,
                semantic_hash="d" * 64,
                raw_payload={"raw": "foreign secret"},
                normalized_projection=_normalized_projection("baseline_v1", now),
            ),
        )
        session.add_all(
    ...
    _install_principals(
        ...
        {
            "reader": _principal(["context.read"]),
            "denied": _principal(["devices.read"]),
        },
    )
    app = create_app(_settings(), session_provider)
    async with httpx.AsyncClient(
        ...
        response = await client.get(
            ...
            params={"profile": "baseline_v1", "limit": 2},
            headers={"Authorization": "Bearer reader"},
        )
        denied = await client.get(
            f"/api/v1/devices/{device.id}/context/snapshots",
        ...
        diagnostic_rejected = await client.get(
            f"/api/v1/devices/{device.id}/context/snapshots",
            params={"profile": "diagnostic_v1"},
            headers={"Authorization": "Bearer reader"},
        )
        health_rejected = await client.get(
            f"/api/v1/devices/{device.id}/context/snapshots",
...
async def test_safe_device_last_seen_comes_from_latest_session_only(
    ...
    async with session_provider() as session:
        device = Device(
            id=uuid4(),
            device_identifier="presence-device",
            display_name="Presence",
            retired_at=None,
        )
        other = Device(
            id=uuid4(),
            device_identifier="presence-other",
            display_name="Other",
            retired_at=None,
        )
        session.add_all(
            (
                ...
                DeviceSession(
                    ...
                    session_identifier="presence-old",
                    created_at=older,
                    expires_at=now,
                ),
                DeviceSession(
                ...
                DeviceSession(
                    ...
                    device_id=other.id,
                    device_instance_id=None,
                    session_identifier="presence-foreign",
                    created_at=now.replace(year=2026),
                    expires_at=now,
                ),
            )
        )
    ...
    async with httpx.AsyncClient(
        ...
        context = await client.get(
            f"/api/v1/devices/{device.id}/context",
            headers={"Authorization": "Bearer context-reader"},
        )
    listed_device = next(
...
async def test_list_devices_reads_latest_sessions_in_one_query_without_duplicate_ties(
    ...
    """A per-device session query would turn this public listing into N+1 I/O."""
    seen_at = datetime(2026, 7, 30, 10, 0, tzinfo=UTC)
    first_id, second_id = sorted((uuid4(), uuid4()), key=str)
    async with session_provider() as session:
        tied = Device(
            id=uuid4(), device_identifier="a-tied", display_name="Tied", retired_at=None
        )
        without_session = Device(
            id=uuid4(),
            device_identifier="b-empty",
            display_name="Empty",
            retired_at=None,
        )
        session.add_all(
    ...
    engine = session_provider.kw["bind"].sync_engine
    ...
    assert [item["device_identifier"] for item in listed] == ["a-tied", "b-empty"]
    assert listed[0] | {"last_seen_at": None} == {
        "id": str(tied.id),
        "device_identifier": "a-tied",
        "display_name": "Tied",
        "retired_at": None,
        "last_seen_at": None,
    }
...
async def test_context_comparison_rejects_duplicate_snapshot_id(
    ...
    async with session_provider() as session:
        device = Device(
            id=uuid4(),
            device_identifier="compare-device",
    ...
    async with httpx.AsyncClient(
        ...
        response = await client.get(
            f"/api/v1/devices/{device.id}/context/snapshots/compare",
            params={
                "before_snapshot_id": str(snapshot.id),
                "after_snapshot_id": str(snapshot.id),
            },
            headers={"Authorization": "Bearer reader"},
        )
