...
from pc_agent.core.orchestrator_collect_helpers import (
    handle_collect as helper_handle_collect,
)
...
try:
    ...
    ModuleNotFoundError
):  # pragma: no cover - defensive path for nested cwd entrypoints
...
def _archive_stale_pending_update(
    pending_path: pathlib.Path, payload: Dict[str, Any], *, incoming_version: str
) -> None:
    ...
    archive_path.write_text(
        json.dumps(archived_payload, ensure_ascii=False, indent=2), encoding="utf-8"
    )
...
class AgentOrchestrator:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if (
                hasattr(self, "pending_tool_calls")
            ...
            if self.module_manager:
                ...
                try:
                    ...
                    for m in installed.get("modules", []):
                        ...
                        try:
                            ...
                            inst = self.loader.load_module_from_path(
                                module_name, m_path, entrypoint=entrypoint
                            )
                            ...
                            try:
                                self.module_manager.remove_version_force(
                                ...
                                logger.warning(
                                    f"Не удалось удалить сломанный модуль {module_name}@{module_version}: {rm_e}"
                                )
                            else:
                                broken_modules_deleted.append(
                    ...
                    if active_modules_loaded > 0:
                        ...
                        logger.info(
                            "В modules_store не найдено активных модулей для автозагрузки"
                        )
                except Exception as e:
                    logger.error(
                        f"Ошибка при автозагрузке активных модулей из modules_store: {e}"
                    )
                    logger.exception(e)
            else:
                logger.debug(
                    "ModuleManager не инициализирован, пропускаем автозагрузку из modules_store"
                )
            ...
            if self.db_manager:
                ...
                if broken_modules_deleted:
                    startup_reason += (
                        f";broken_removed={','.join(broken_modules_deleted)}"
                    )
    ...
    async def handle_command(self, command: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if payload_request_id:
            ...
            logger.debug(
                f"request_id не указан в payload, сгенерирован новый: {request_id}"
            )

        device_id = command.get("device_id")
        ticket_id = command.get("ticket_id") or (command.get("params", {}) or {}).get(
            "ticket_id"
        )
        actor_role = command.get("actor_role")
        agent_id = (
            self.agent_uuid if hasattr(self, "agent_uuid") and self.agent_uuid else None
        )
        ...
        try:
            match cmd:
                ...
                case "activate_module":
                    ...
                    result = await self._handle_activate_module(
                        name, version, actor_role, meta
                    )
                ...
                case "deactivate_module":
                    name = command.get("name")
                    actor_role = command.get("actor_role", "user")
                    result = await self._handle_deactivate_module(
                        name, actor_role, meta
                    )
                ...
                case "remove_module_version":
                    ...
                    version = command.get("version")
                    actor_role = command.get("actor_role", "user")
                    result = await self._handle_remove_module_version(
                        name, version, actor_role, meta
                    )
                ...
                case "install_module_package":
                    ...
                    actor_role = command.get("actor_role", "user")
                    replace_if_different_sha = command.get(
                        "replace_if_different_sha"
                    ) or (command.get("params") or {}).get(
                ...
                case "cancel_operation":
                    ...
                    target_operation_id = params.get(
                        "target_operation_id"
                ...
                case "run_tool" | "call_tool":
                    ...
                    result = await self._handle_run_tool(
                        tool, command_params, actor_role, meta
                    )
                ...
                case "run_recipe":
                    ...
                    bridge_result = await RecipeRunnerBridge(
                        self.module_manager, self.loader
                    ).run_recipe(
                        recipe_payload,
                        runtime_context,
                    )
                ...
                case "start_job":
                    ...
                    params = command.get("params", {})
                    actor_role = command.get("actor_role", "user")
                    result = await self._handle_start_job(
                        job_type, params, actor_role, device_id, meta
                    )
                ...
                case "job_send_event":
                    ...
                    chat_job_id = command.get("job_id") or command.get(
                        "params", {}
                    ).get("job_id")
                    event = command.get("event") or command.get("params", {}).get(
                        "event"
                    )
                    actor_role = command.get("actor_role", "user")
                    result = await self._handle_job_send_event(
                        chat_job_id, event, actor_role, meta
                    )
                ...
                case "consent_decision":
                    ...
                    result = await self._handle_consent_decision(
                        consent_token, approved, session_key, meta
                    )
                ...
                case "ui_notify":
                    ...
                    if not isinstance(ev, dict):
                        ...
                        if self.ui_bus:
                            ...
                            logger.info(
                                f"UI notify published: {ev.get('event')} job_id={ev.get('job_id')}"
                            )
                        else:
                            logger.warning(
                                "ui_notify получен, но ui_bus не инициализирован"
                            )
                        result = ok(
                            data=ToolData(observations={"published": True}), meta=meta
                        )
                ...
                try:
                    ...
                    if result.error:
                        error_json = json.dumps(
                            result.error.model_dump(), ensure_ascii=False
                        )
                    ...
                    if cmd == "collect":
                        logger.info(
                            f"Collect command completed: outbox_written=1 (single canonical ToolResponse)"
                        )
                    logger.info(
                        f"Завершен job: {command_job_id}, status={result.status}"
                    )
                except Exception as db_error:
            ...
            if result.status == "success":
                logger.success(
                    f"OK Команда '{cmd}' выполнена успешно (duration: {duration_ms}ms)"
                )
            else:
                logger.warning(
            ...
            if self.db_manager and job_created:
                try:
                    error_json = (
                        json.dumps(result.error.model_dump(), ensure_ascii=False)
                        if result.error
                        else None
                    )
                    await self.db_manager.finish_job(
    ...
    async def _handle_ping(self, meta: ToolMeta) -> ToolResponse:
        ...
        try:
            ...
            agent_uuid = (
                self.agent_uuid
                if hasattr(self, "agent_uuid") and self.agent_uuid
                else None
            )
    ...
    async def _handle_collect(
        self, modules: Optional[List[str]], meta: ToolMeta
    ) -> ToolResponse:
        ...
        try:
            ...
            for module in self.loaded_modules:
                module_info = {
                    ...
                    "description": module.__class__.__doc__.strip()
    ...
    async def _handle_list_installed_modules(self, meta: ToolMeta) -> ToolResponse:
        try:
            ...
            observations = {"modules": data.get("modules", [])}
            return ok(data=ToolData(observations=observations), meta=meta)
        except Exception as e:
            return fail(
                code="LIST_INSTALLED_FAILED", message=str(e), meta=meta, retriable=True
            )
    ...
    async def _handle_activate_module(
        self,
        name: Optional[str],
        ...
        try:
            ...
            if actor_role != "admin":
                return fail(
                    code="FORBIDDEN", message="admin only", meta=meta, retriable=False
                )

            if not name or not version:
                return fail(
                    code="ACTIVATE_FAILED",
                    message="name and version required",
                    meta=meta,
                )
            ...
            return fail(
                code="ACTIVATE_FAILED", message=str(e), meta=meta, retriable=True
            )
    ...
    async def _rebuild_registry_from_active_modules(self) -> None:
        ...
        if self.enabled_modules:
            ...
            for module in builtin_modules:
                self.registry.register(module)
                logger.debug(
                    f"Встроенный модуль '{module.name}' зарегистрирован в реестре"
                )
        ...
        if self.module_manager:
            ...
            for m in installed.get("modules", []):
                ...
                try:
                    ...
                    self.registry.register(inst)
                    logger.debug(
                        f"Package модуль '{module_name}' зарегистрирован в реестре"
                    )
                except Exception as load_err:
                    # Модуль на диске не загружается — удаляем, чтобы не оставался «установленным, но сломанным»
                    logger.warning(
                        f"Модуль {module_name}@{module_version} не загружается: {load_err}, удаляю с диска"
                    )
                    try:
                        self.module_manager.remove_version_force(
                            module_name, module_version
        ...
        try:
            ...
            if (
                hasattr(self, "_last_toolset_hash")
                and self._last_toolset_hash == new_toolset_hash
            ):
                logger.debug(
                    f"toolset_hash не изменился ({new_toolset_hash}), пропускаем tools_changed"
                )
            ...
            if self.db_manager and self.device_id:
                ...
                logger.info(
                    f"tools_changed event enqueued: toolset_hash={new_toolset_hash}, tools_count={tools_count}"
                )
        except Exception as e:
    ...
    def _purge_module_runtime(self, module_name: Optional[str]) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Failed to unregister module '{module_name}' from registry",
                exc_info=True,
            )
        if self.loader:
            try:
                ...
                logger.debug(
                    f"Failed to unload runtime cache for module '{module_name}'",
                    exc_info=True,
                )
    ...
    def _get_module_source_path(self, module_instance: Any) -> Optional[Path]:
        if module_instance is None:
            return None
        try:
            source_path = inspect.getsourcefile(
                module_instance.__class__
            ) or inspect.getfile(module_instance.__class__)
        except Exception:
            return None
        if not source_path:
            return None
    ...
    def _get_expected_tool_method_from_active_manifest(
        ...
        short_tool_name = (
            full_tool_name.split(".", 1)[1] if "." in full_tool_name else full_tool_name
        )
    ...
    async def _ensure_module_runtime_matches_inventory(
        ...
        *,
        ...
        active_path, expected_method = (
            self._get_expected_tool_method_from_active_manifest(
                module_name,
        ...
        registry_tool = (
            self.registry.get_tool(full_tool_name) if full_tool_name else None
        )
        ...
        if active_path:
            ...
            if loaded_instance is None:
                ...
                and active_path_resolved not in source_path.parents
            ):
                should_rebuild = True
            ...
            if full_tool_name and expected_method:
                current_method = (
                    registry_tool.get("method_name") if registry_tool else None
                )
            ...
            if loaded_instance is not None and self._is_dynamic_module_instance(
                loaded_instance
            ):
                should_rebuild = True
    ...
    async def _emit_module_state_changed(self, reason: str = "unknown") -> None:
        ...
        try:
            ...
            logger.info(
                f"[module_state_changed] Event enqueued: reason={reason} modules={len(snapshot)}"
            )
        except Exception as e:
            logger.warning(f"[module_state_changed] Failed to enqueue event: {e}")

    async def _handle_rollback_module(
        self, name: Optional[str], actor_role: str, meta: ToolMeta
    ) -> ToolResponse:
        """
        ...
        if not prev_path:
            return fail(
                ...
                message="No previous version to rollback",
                meta=meta,
                retriable=False,
            )
    ...
    async def _handle_deactivate_module(
        self, name: Optional[str], actor_role: str, meta: ToolMeta
    ) -> ToolResponse:
        """
    ...
    async def _handle_remove_module_version(
        ...
        if not name or not version:
            return fail(
                code="REMOVE_FAILED", message="name and version required", meta=meta
            )
        ...
        try:
            ...
            if removed:
                remaining_modules = {
                    module_info["name"]
                    for module_info in self.module_manager.list_installed().get(
                        "modules", []
                    )
                }
                if name not in remaining_modules:
                    ...
                    await self._emit_module_state_changed(
                        reason=f"remove_version:{name}@{version}"
                    )
                return ok(
                    data=ToolData(observations={"removed": f"{name}@{version}"}),
                    meta=meta,
                )
            else:
                return fail(
            ...
            return fail(
                code="REMOVE_FAILED", message=str(e), meta=meta, retriable=False
            )
        ...
        if target_operation_id in self.running_tasks:
            ...
            if event_ticket_id and self.db_manager:
                try:
                    ...
                    logger.error(
                        f"[cancel_operation] Failed to publish canceled event: {e}"
                    )
            ...
            finalized_local = await self._finalize_local_canceled_command(
                target_operation_id, meta
            )
            ...
            if cancel_status == "canceled" and event_ticket_id and self.db_manager:
                try:
                    ...
                    logger.error(
                        f"[cancel_operation] Failed to publish queued canceled event: {e}"
                    )
    ...
    async def _finalize_local_canceled_command(
        self, target_operation_id: str, meta: ToolMeta
    ) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"[cancel_operation] Failed to inspect seen_commands for {target_operation_id}: {exc}"
            )
            return False
        ...
        *,
        ...
        try:
            ...
            if download_url:
                ...
                try:
                    zip_bytes = await self._download_module_zip(
                ...
                try:
                    ...
                    logger.info(
                        f"📦 Модуль '{name}' версии '{version}' получен через base64"
                    )
                except Exception as e:
            ...
            try:
                ...
                if "already installed" in err_str.lower():
                    ...
                    try:
                        ...
                        logger.info(
                            f"Модуль '{name}' версии '{version}' уже установлен, активируем и загружаем"
                        )
            ...
            if result["module_name"] != name:
                return fail(
                    ...
                    details={
                        "expected_name": name,
                        "actual_name": result["module_name"],
                    },
                )
            ...
            if result["module_version"] != version:
                return fail(
                    ...
                    details={
                        ...
                        "actual_version": result["module_version"],
                    },
                )

            if already_installed:
                logger.success(
                    f"Модуль '{name}' версии '{version}' уже установлен, активация и загрузка выполнены"
                )
            else:
            ...
            try:
                active_path = self.module_manager.activate(name, version)
                logger.info(
                    f"Модуль '{name}' версии '{version}' активирован: {active_path}"
                )
            except Exception as e:
                ...
                try:
                    ...
                    logger.info(
                        f"Откат: версия {name}@{version} удалена с диска после сбоя активации"
                    )
                except Exception as rollback_e:
                    logger.warning(
                        f"Не удалось удалить каталог при откате: {rollback_e}"
                    )
                return fail(
                    code="INSTALL_FAILED",
                    message=error_msg,
            ...
            try:
                ...
                self.loader.load_module_from_path(
                    name, active_path, entrypoint=entrypoint
                )
                ...
                try:
                    self.module_manager.remove_version_force(name, version)
                    logger.info(
                        f"Откат: версия {name}@{version} удалена с диска после сбоя загрузки"
                    )
                except Exception as rollback_e:
                    logger.warning(
                        f"Не удалось удалить каталог при откате: {rollback_e}"
    ...
    async def _download_module_zip(
        ...
        if self.identity_manager and self.identity_manager.has_token:
            ...
            if token:
                headers["Authorization"] = f"Bearer {token}"
                logger.debug(
                    f"[DownloadModule] Using token for download: {token[:8]}..."
                )
            else:
                logger.warning(
                    "[DownloadModule] Identity manager has token flag but token is None"
                )
        else:
            logger.warning(
                "[DownloadModule] No token available for download authentication"
            )
    ...
    async def _handle_exec_script(
        self, code: Optional[str], actor_role: str, meta: ToolMeta
    ) -> ToolResponse:
        """
        ...
        try:
            ...
            if get_config().security.allow_remote_code != True:
                return fail(
                    code="REMOTE_CODE_DISABLED",
                    message="Remote code execution disabled",
                    meta=meta,
                    retriable=False,
                )
            ...
            observations = {"result": result, "modules_available": modules_added}
    ...
    async def _handle_describe_tool(
        self, tool_name: Optional[str], meta: ToolMeta
    ) -> ToolResponse:
        """
    ...
    def _redact_params(
        self, tool: Optional[str], params: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _redact_payload(
        self, payload: Any, *, redact_fields: Optional[List[str]] = None
    ) -> Any:
        ...
        if isinstance(payload, dict):
            ...
            for key, value in payload.items():
                ...
                if any(marker in key_lower for marker in sensitive_fields):
                    ...
                    redacted[key] = self._redact_payload(
                        value, redact_fields=list(sensitive_fields)
                    )
            return redacted
        if isinstance(payload, list):
            return [
                self._redact_payload(item, redact_fields=list(sensitive_fields))
                for item in payload
            ]
        return payload
    ...
    async def _publish_chat_event(
        ...
        try:
            ...
            if not event_device_id:
                logger.error(
                    f"[chat_event] missing device_id for job_id={job_id}, event={ev.get('event')}"
                )
                return
            await self.db_manager.enqueue_job_event(
                job_id=job_id,
            ...
            logger.debug(
                f"[chat_event] enqueued job_id={job_id} ticket_id={event_ticket_id} event={ev.get('event')}"
            )
        except Exception as e:
    ...
    async def _publish_screen_ui_done(self, tool: str, operation_id: str) -> None:
        ...
        event_type = (
            "screen_capture_done"
            if tool == "screen.collect"
            else "screen_recording_done"
        )
    ...
    async def _handle_run_tool(
        ...
        # Диагностика
        logger.info(
            f"[AGENT] run_tool tool={tool} chat_job_id={chat_job_id} tool_params_keys={list(tool_params.keys())}"
        )

        logger.info(
            f"[AGENT] run_tool start tool={tool} actor_role={actor_role} request_id={meta.request_id}"
        )
        ...
        try:
            ...
            if module_name:
                await self._ensure_module_runtime_matches_inventory(
                    module_name, full_tool_name=tool
                )
                tool_data_from_registry = (
                    self.registry.get_tool(tool) or tool_data_from_registry
                )
            ...
            if not module_instance:
                error_msg = (
                    f'Экземпляр модуля "{module_name}" не найден в loaded_modules'
                )
                if chat_job_id:
                    await self._publish_chat_event(
                        chat_job_id,
            ...
            if not hasattr(module_instance, method_name):
                ...
                return fail(
                    code="TOOL_NOT_FOUND", message=error_msg, meta=meta, retriable=False
                )
            ...
            if params_model is not None:
                ...
                if ValidationError is None:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Параметры инструмента {tool} успешно валидированы через {params_model.__name__}"
                        )
                        ...
                        return fail(
                            ...
                            message=error_msg,
                            meta=meta,
                            details={"errors": e.errors(), "tool": tool},
                            retriable=False,
                        )
            ...
            metadata_payload = {
                ...
                or (tool.split(".", 1)[0] if "." in tool else "system"),
            ...
            try:
                ...
                logger.warning(
                    f"Ошибка создания ToolMetadata для {tool}: {e}, используем default"
                )
            ...
            if not decision_allow:
                ...
                if decision_requires_consent:
                    ...
                    if self.ui_bus:
                        await self.ui_bus.publish(event)
                        logger.info(
                            f"Событие consent_required опубликовано: consent_token={consent_token}, tool={tool}"
                        )
                    get_action_trace_recorder().record(
                        action_trace,
                        stage="consent_wait",
                    ...
                    return fail(
                        ...
                        or f'Требуется согласие для выполнения инструмента "{tool}"',
                        meta=meta,
                        details={
                ...
                error_msg = (
                    decision_reason
                    or f'Доступ к инструменту "{tool}" запрещен для роли "{actor_role}"'
                )
                if chat_job_id:
                    await self._publish_chat_event(
                        chat_job_id,
            ...
            try:
                ...
                if not has_kwargs:
                    ...
                    params_to_use = {
                        k: v for k, v in params_to_use.items() if k in allowed
                    }
            except Exception:
                pass
            ...
            effective_timeout = metadata.timeout_sec or resources_policy.get(
                "max_runtime_sec"
            )
            ...
            try:
                if effective_timeout:
                    observations = await asyncio.wait_for(
                        task, timeout=float(effective_timeout)
                    )
                else:
                ...
                get_action_trace_recorder().record(
                    ...
                    details={
                        ...
                        "method_name": method_name,
                        "duration_ms": int(
                            (time.perf_counter() - module_execution_started) * 1000
                        ),
                        "operation_id": operation_id,
                    },
                )
            ...
            observations_clean = {
                k: v for k, v in observations.items() if not k.startswith("_")
            }
            ...
            for item in artifact_intents_data:
                if isinstance(item, dict) and "local_path" in item:
                    try:
                        ...
                        logger.debug(
                            f"Добавлен артефакт для загрузки: {item['local_path']}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"Ошибка создания ArtifactIntent для {item.get('local_path')}: {e}"
                        )
            ...
            if (
                isinstance(max_artifact_count, int)
                and max_artifact_count >= 0
                and len(artifact_intents) > max_artifact_count
            ):
                return fail(
            ...
            if artifact_intents:
                try:
                    ...
                    logger.info(
                        f"📤 Начинаю загрузку {len(artifact_intents)} артефактов..."
                    )
                    (
                        uploaded_artifacts,
                        upload_errors,
                    ) = await artifact_manager.upload_many(artifact_intents)

                    logger.success(
                        f"OK Загружено артефактов: {len(uploaded_artifacts)}/{len(artifact_intents)}"
                    )
            ...
            if cleanup_paths:
                ...
                for cleanup_path in cleanup_paths:
                    try:
                        ...
                        logger.warning(
                            f"вљ пёЏ  Не удалось удалить временный файл {cleanup_path}: {e}"
                        )
            ...
            if upload_errors:
                warnings.extend(
                    [f"Ошибка загрузки артефакта: {e.message}" for e in upload_errors]
                )
            ...
            if upload_errors:
                logger.warning(
                    f"[AGENT] run_tool partial tool={tool} duration_ms={duration_ms} artifacts={len(uploaded_artifacts)} upload_errors={len(upload_errors)}"
                )
                get_action_trace_recorder().record(
                    module_action_trace,
                    stage="finish",
                ...
                return partial(
                    data=data, meta=meta, warnings=warnings, errors=upload_errors
                )
            else:
                logger.success(
                    f"[AGENT] run_tool ok tool={tool} duration_ms={duration_ms} artifacts={len(uploaded_artifacts)}"
                )
                get_action_trace_recorder().record(
                    module_action_trace,
                    stage="finish",
            ...
            logger.error(
                f"[AGENT] run_tool fail tool={tool} code=COMMAND_FAILED exc={type(e).__name__}"
            )
            logger.exception(e)
    ...
    async def _handle_consent_decision(
        ...
        try:
            ...
            if consent_record.state == ConsentState.APPROVED:
                ...
                logger.info(
                    f"OK Согласие получено, выполняю tool: {tool_name}, consent_token={consent_token}"
                )
                ...
                if result.status == "success" and result.data:
                    observations = (
                        result.data.observations if result.data.observations else {}
                    )
                    # Берем первые 200 символов результата как preview
                    result_str = str(observations)[:200]
                    result_preview = result_str + (
                        "..." if len(str(observations)) > 200 else ""
                    )
                elif result.error:
                    result_preview = (
                ...
                if self.ui_bus:
                    await self.ui_bus.publish(event)
                    logger.info(
                        f"Событие tool_executed опубликовано: consent_token={consent_token}, tool={tool_name}"
                    )
                get_action_trace_recorder().record(
                    consent_trace,
                    stage="response",
                ...
                logger.info(
                    f"ERROR Согласие отклонено: consent_token={consent_token}, tool={tool_name}"
                )
                consent_trace.tool_name = tool_name
                consent_trace.ticket_id = pending_ticket_id
                ...
                if self.ui_bus:
                    await self.ui_bus.publish(event)
                    logger.info(
                        f"Событие tool_denied опубликовано: consent_token={consent_token}, tool={tool_name}"
                    )
                get_action_trace_recorder().record(
                    consent_trace,
                    stage="response",
    ...
    async def _handle_start_job(
        ...
        return await helper_handle_start_job(
            self, job_type, params, actor_role, device_id, meta
        )

    async def _handle_stop_job(
        self, job_id: Optional[str], actor_role: str, meta: ToolMeta
    ) -> ToolResponse:
...
async def test_tool_response_format():
    ...
    try:
        ...
        result = await orchestrator.handle_command(
            {"cmd": "ping", "request_id": "test-request-1"}
        )
        assert result["status"] in ["success", "error", "partial"], (
            f"Неверный статус: {result.get('status')}"
        )
        ...
        assert result["meta"]["command"] == "ping", (
            f"Неверная команда: {result['meta']['command']}"
        )
        ...
        if result["status"] == "success":
            ...
            assert "message" in result["data"]["observations"], (
                "Отсутствует message в observations"
            )
            assert "agent" in result["data"]["observations"], (
                "Отсутствует agent в observations"
            )
        ...
        assert "data" in result, "Отсутствует поле data"
        assert "observations" in result["data"], "Отсутствует observations в data"
        assert "modules" in result["data"]["observations"], (
            "Отсутствует modules в observations"
        )
        assert isinstance(result["data"]["observations"]["modules"], list), (
            "modules должен быть списком"
        )
        ...
        assert "data" in result, "Отсутствует поле data"
        assert "observations" in result["data"], "Отсутствует observations в data"
        assert "results" in result["data"]["observations"], (
            "Отсутствует results в observations"
        )
        assert isinstance(result["data"]["observations"]["results"], dict), (
            "results должен быть словарем"
        )
        ...
        assert result["error"]["code"] == "UNKNOWN_COMMAND", (
            f"Неверный код ошибки: {result['error']['code']}"
        )
        ...
        result = await orchestrator.handle_command(
            {"cmd": "collect", "modules": ["nonexistent_module"]}
        )
        ...
        logger.success(
            "OK collect с несуществующим модулем возвращает partial с warnings"
        )
...
"""
        result = await orchestrator.handle_command(
            {"cmd": "exec_script", "code": test_code}
        )
        assert result["status"] == "success", f"Неверный статус: {result.get('status')}"
        assert "data" in result, "Отсутствует поле data"
        assert "observations" in result["data"], "Отсутствует observations в data"
        assert "result" in result["data"]["observations"], (
            "Отсутствует result в observations"
        )
...
async def test_orchestrator():
    ...
    try:
        # Создаем оркестратор с тестовыми модулями
        orchestrator = AgentOrchestrator(
            db_manager=db_manager, enabled_modules=["system"]
        )
