...
async def test_noneligible_primary_failures_never_call_legacy(status: int) -> None:
    ...
    async def legacy_fetch() -> object:
        nonlocal calls; calls += 1; return {}
    adapter, _ = _adapter(_Response(status, ""), legacy_fetch=legacy_fetch)
    assert (await adapter.fetch_recommendation(platform="windows_amd64", channel="stable")).source == "endpoint"
    assert calls == 0
...
async def test_connection_failure_calls_legacy_once() -> None:
    calls = 0
    class Session:
        def get(self, url: str, *, headers: dict[str, str]): raise aiohttp.ClientConnectionError()
    async def legacy_fetch() -> object:
        nonlocal calls; calls += 1; return {}
...
async def test_terminal_report_reuses_report_key_after_failed_post(tmp_path) -> None:
    class Session:
        ...
        def get(self, url: str, *, headers: dict[str, str]): raise AssertionError
        def post(self, url: str, *, headers: dict[str, str], json: dict[str, object]): self.bodies.append(json); return _Response(500, "")
    first_session, second_session = Session(), Session()
    first = EndpointUpdateAdapter(api_url="https://endpoint.example.test", bearer_token=lambda: "token", session=first_session, data_root=tmp_path)
    second = EndpointUpdateAdapter(api_url="https://endpoint.example.test", bearer_token=lambda: "token", session=second_session, data_root=tmp_path)
    args = ("caa31a48-bf2f-4f1c-8b77-d1be77e12b4e",)
    kwargs = {"status": "failed", "reported_version": "1.2.3", "safe_code": "launcher_apply_failed"}
    assert await first.report_terminal(*args, **kwargs) is False
    assert await second.report_terminal(*args, **kwargs) is False
    assert first_session.bodies[0]["report_key"] == second_session.bodies[0]["report_key"]
    record = json.loads((tmp_path / "updates" / "endpoint_update_reports.json").read_text())[0]
    assert set(record) == {"operation_id", "report_key", "status", "reported_version", "safe_code", "delivered_at"}
