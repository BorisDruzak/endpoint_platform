...
| `pc_agent/core/identity.py` | Каноническая identity агента: `machine_id` -> `device_id`, вторичный `install_id`, миграция legacy `identity.json`, handshake metadata включая `device_fingerprint`; server-side legacy/provisioning token rebind is rejected before token binding changes when the target device fingerprint mismatches |
...
| Файл | Назначение |
|------|------------|
| `pc_agent/ui_bridge/api_server.py` | Локальный API для GUI (SSE/HTTP), settings + local agent control/diagnostics (`/ui/agent/status`, `/ui/agent/logs`, `/ui/agent/shutdown`, `/ui/agent/update`); `/ui/automation/status` returns HTTP 200 with `status=unavailable` when the GUI automation provider has not attached yet, so fresh-instance checks can distinguish a live bridge from a configured GUI controller; `/ui/automation/run` preserves structured downstream server `4xx` errors from GUI ticket actions instead of wrapping expected validation/auth/not-found/conflict denials as local `500`; `ticket.create` forwards the same Service Catalog identifiers as the GUI wizard (`request_template_key`, `service_code`, `offering_code`, `offering_full_code`) so automation probes exercise persisted catalog dimensions instead of legacy form-only create |
...
2026-05-23/2026-06-08/2026-06-16/2026-06-18: `pc_agent/ui_gui/dynamic_form_widget.py` is the reusable schema/field renderer for ticket creation and the other-account exception form, not for normal local profile registration. `chat_panel.py` keeps compatibility aliases for `TicketDynamicFieldsWidget`; backend registration endpoints remain compatibility API surface for older agents, but `main_window.py` no longer builds a legacy registration entry page, `default_agent_registration_form()`, or local submit/confirm registration handlers. Account login is a server-backed requester session and not a replacement for the machine token: `pc_agent/ui_gui/account_gate.py` is the first non-modal page, settings stay accessible before login, and ticket list/create/detail are gated until `AccountSessionManager` has a valid server-issued session from confirmed binding, PA8 server-validated GUI password login, or admin-approved verified other-account login. Browser-first login/registration buttons create `device_browser_pairings` through `pc_agent/ui_gui/server_api.py`, open `/app/device/login` or `/app/device/register`, then `main_window.py` polls the agent pairing endpoint; login saves the one-time server `session_token` only after agent pickup, and auto-approved registration now accepts the same one-time confirmed-binding `session_token` directly from registration pickup before falling back to account-state refresh/session creation; manual approval stays on the registration-pending gate. PA8 local password login calls `TicketApiClient.create_gui_password_account_session()` against `/api/registry/agent/account-sessions/login`, clears the password field, stores only server session id/token, and redacts password-like trace fields. Since 2026-06-16, normal agent UI has no legacy full-profile registration button/form/confirm action, shows `Привязать через браузер`, exposes the pairing code for copy, and routes accidental legacy registration view names back to the account gate. In agent `3.1.67`, `ChatPanel` also stops reading/writing local requester profiles, no longer opens the legacy profile manager/sidebar, no longer syncs self-reported GUI profiles to registry, and automation no longer exposes `profile.upsert` / `profile.select`. A `registration_pending` session is stored only to show/poll the pending registration gate and is not a ticket-workspace login. Pending other-account request sessions are also gate-only; terminal request statuses (`rejected`, `expired`, `canceled`) clear the local pending gate and return to current account-state. Before login, `main_window.py` enters account entry mode: it hides the main sidebar/footer shell, shows only the account gate or settings by default, gives entry settings an explicit back action, and opens the full workspace only after confirmed/verified-other-account session selection; after login the Account page is a read-only server session summary with logout, not the legacy local profile editor.
...
2026-06-18: Browser registration pairing in `main_window.py` now accepts a confirmed-binding `session`/`session_token` directly from the registration pickup response when the server auto-approved the first non-conflicting binding, saves it locally and opens the ticket workspace. If the pickup response is from an older server without that token, the GUI still checks fresh account-state, creates a confirmed-binding session when possible, or converts the consumed `claim_id` into a server-issued `registration_pending` account session through `TicketApiClient.create_registration_pending_account_session()` when policy leaves the claim pending. It keeps the user on the pending registration gate until admin approval and preserves the active pairing code/link plus registration summary if the follow-up account-state or pending-session refresh fails after browser confirmation. A later account-state refresh that sees a loginable `confirmed_binding` while no local account session is active also creates the confirmed-binding session automatically, so a browser-confirmed device does not remain stuck on the account gate waiting for a manual "log in as" click; release `3.1.72` carries this delayed confirmation recovery and must not overwrite `3.1.71` artifacts. Release `3.1.73` additionally refreshes stale local confirmed-binding profile fields from account-state after the requester completes the web profile, so the GUI account card switches from the original login/email snapshot to the current registry display/full name without forcing logout. Long web registration/profile-completion polling also keeps the pairing code/link visible and returns a refresh-oriented pending message instead of the generic account-check error. `account_gate.py` also shows the browser handoff URL directly in the gate for Linux/no-browser fallback and hides the repeat browser-registration action while a pairing is active. Release `3.1.75` adds the direct `Веб кабинет` action and fallback links to `/app/requester`, `/app/login`, and `/app/login?forgot_password=1` from the account gate.

2026-06-19: `AccountGateWidget` and `MainWindow` expose a direct `Веб кабинет` action for users who do not have the cabinet URL outside the agent, both on the account gate and in the authenticated sidebar. The gate shows fallback links for `/app/requester`, `/app/login` and `/app/login?forgot_password=1`; `main_window.py` derives those URLs through the same HTTPS web-boundary normalization used by browser-pairing handoffs and opens `/app/requester` when the button is pressed. These links are entry/help navigation only and do not create local requester profiles, machine tokens or account sessions. Release `3.1.76` also makes startup/manual self-update visible: `ws_agent.py` overlays `update_request_state=applying` before update shutdown, while the GUI shows `pending_restart` / `applying` / `restarting` as progress instead of a silent window close.
...
| Файл | Назначение |
|------|------------|
| `pc_agent/ui_gui/main_window.py` | Главное окно (sidebar + dashboard/tickets/account/create/settings stack), настройки и статусы, authenticated sidebar `Веб кабинет` action, sidebar agent-status/update card as the visible online/version/update surface, auto-collapsing sidebar icon rail for focused ticket/create-ticket workspaces, create-ticket navigation switches the workspace synchronously and uses a GUI task helper for async preparation from Qt slots, protected-connection footer under the workspace, секция always-on/tray/logging diagnostics, release badge и кнопка recommended update из локального `ui_bridge`; account gate is shown first in entry mode, which hides the main sidebar/footer shell until login, settings remain available without login with an explicit back button, and the normal GUI no longer builds the legacy registration entry page or local full-profile registration submit/confirm handlers. Browser registration stays in account gate through pairing code/direct link only. The post-login Account page shows the selected server-issued account session plus logout instead of the legacy profile selector/editor, refreshes missing or stale display/full/email/phone fields from account-state for existing local sessions, validates stored server sessions through `/api/registry/agent/account-sessions/{session_id}/validate` on refresh/startup, clears local session/ticket state and returns to account gate when validation reports `ACCOUNT_SESSION_*` invalidation such as revoked sessions or inactive bindings, retrieves the restart-safe one-time token for approved other-account requests before saving the local session, handles PA8 GUI password login by saving only a returned `confirmed_binding` session id/token and stripping password-like payload fields, and dashboard/tickets/create/account pages are unavailable until local account session is valid; create wizard refreshes on both `ticketFormPackChanged` and `serviceCatalogChanged`, so catalog-only publication/sync updates render new service/offering cards without reopening the GUI; connection/account/ticket shell state exposes stable UIA-readable accessibility names for live validation without leaking tokens; settings page shows always-on core modules separately from modules_store packages so `inventory` / `presence` are visible even when they are not installed packages; update CTA now renders explicit `requesting` / `requested` / `pending_restart` / `applying` / `restarting` states, shows a progress dialog while the agent prepares to restart for update, and runs a short refresh burst after an accepted local update request; connection rejected/block events show the server-provided message, including token-limit diagnostics; update/status microcopy is localized for end users; large Qt soft shadows are opt-in through `PC_AGENT_ENABLE_GUI_SHADOWS=1` to avoid focused-window CPU spikes on weak Windows graphics stacks; recording STOP overlay stays a `Qt.Tool` window so it does not create extra `pc_agent` Taskbar/Alt-Tab entries; sidebar/header widgets must be parented or added to layouts before they are shown so blank labels do not become separate `python` top-level windows |
...
| `pc_agent/ui_gui/account_gate.py` | Non-modal first-screen account gate: displays verified server account-state, prefers browser login/registration actions backed by `device_browser_pairings`, exposes a copyable pairing code after browser handoff, and has no local full-profile registration/confirm controls in the normal GUI. It keeps local confirmed-binding login, PA8 login/password form for bound-account candidates, and request-admin-approval-for-other-account actions; password submit emits login/password once, clears the password field and relies on the server to validate/bind, while local pending other-account requests block the gate only while non-terminal, and terminal statuses fall back to fresh account-state; approved `verified_other_account` server sessions are selectable, while unregistered/pending/unknown states do not show the other-account action; other-account login never calls registration endpoints and every ticket uses the server-issued account session instead of trusted local identity payloads |
...
| `pc_agent/ui_gui/automation_controller.py` | Local `/ui/automation/run` GUI/runtime controller for repeatable live checks: account refresh/login actions use the same account-gate flow as the GUI, status reports `has_account_session` instead of a legacy active-profile flag, create-wizard snapshot/select-catalog actions expose deterministic service/offering state, `ticket.create` sends the selected server-issued account session to the ticket API without a local profile precheck, and `ticket.create`/`ticket.open` restart detail polling with `TICKET_DETAIL_POLL_INTERVAL_MS` rather than a fresh `QTimer.interval()` so automation cannot create a zero-interval GUI busy timer. |
| `pc_agent/ui_gui/chat_panel.py` | Чат, создание обращения, reply-to, mark-read; ticket list/create/detail/message/read/upload/close APIs are gated by the `account_session_provider`, requester summaries and `create_ticket()` use the selected server account session only, sending only `session_id`/optional token for requester account validation. The normal GUI no longer reads/writes `requester_profiles.json`, opens the legacy profile manager/sidebar, exposes a local active-profile gate, or calls `sync_registry_profile()` from `ChatPanel`; without a web account session, create-ticket surfaces stay blocked. Создание заявки использует request-template catalog plus P1/P1.1 Service Catalog cache (`service_catalog.json`) from `/api/service-catalog/current`, keeps legacy form-pack fallback, shows explicit `Раздел обращения -> Тип обращения` catalog cards before the dynamic form, binds the selected offering to `request_template_key`, and sends `service_code`/`offering_code`/`offering_full_code` in server preview/create without changing Protocol V3; `serviceCatalogChanged` is emitted when the normalized requester-safe catalog changes, independent of form-pack version updates, so published Studio offerings can appear in an already-created wizard page; if catalog is unavailable, it falls back to legacy form cards and still supports unique-template catalog enrichment. It caches server form packs with `request_template_version`, `form_schema_id`/`form_schema_version`, `*_policy_code` and `policy_refs`, and refreshes the cache when server version or normalized policy metadata changes; preserves field-level `visibility`/`process_mapping` metadata and hides requester-invisible internal process fields unless the template explicitly exposes them; renders server-driven priority fields from `field_roles`/`priority_policy`, supports extended form field types including `multi_select`, native `date`/`datetime` controls, registry-backed `user/department/location/device/service_picker` combos and replaceable/clearable `file` fields that contribute attachment paths; `service_picker` remains CMDB affected-service selection, not catalog service selection. |
...
| `pc_agent/ui_gui/server_api.py` | Обращение к серверу из GUI, создание обращений через `/api/tickets/create` с legacy `form_key`, first-class `request_template_key`, optional P1 Service Catalog fields (`service_code`, `offering_code`, `offering_full_code`), optional `diagnostic_consent` and server-backed `requester_account` session id; GUI request headers refresh the active local auth token from `storage.db` before requester API calls so runtime reprovision/token rotation cannot leave stale Bearer headers in an already-open GUI; ticket-bound methods raise structured `ServerApiError` / `ServerApiConnectionError` for downstream HTTP/network failures so the local automation bridge can preserve `http_status`, `error_code` and validation `details` without logging raw session tokens/passwords; `get_service_catalog_current()` reads requester-safe `/api/service-catalog/current`; server-backed preview through `/api/tickets/create/preview`, отправка `reply_to`/message metadata, вызов `mark_ticket_read()` и ticket history API с `get_ticket(..., since_event_id=..., before_event_id=..., limit=...)`; ticket list/detail/message/read/upload/close methods pass account session id/token via query/body/headers so server authorization is account-scoped, not device-only; `sync_registry_profile()` keeps the legacy profile sync endpoint compatible, registration profile submission/status/confirm/account-state, server-side account sessions, PA8 GUI password login and browser pairings use `/api/registry/agent/profile`, `/api/registry/agent/registration-status`, `/api/registry/agent/claims/{claim_id}/confirm`, `/api/registry/agent/account-state`, `/api/registry/agent/account-sessions/*`, `/api/registry/agent/account-sessions/login`, `/api/registry/agent/account-login-requests/*` and `/api/registry/agent/browser-pairings*` with normalized `{status:"success", data}` unwrapping and friendly HTTP auth error messages for GUI callers, а `get_registry_options()` читает `/api/registry/options` для picker-полей формы |
...
- **command_result replay / restart recovery** — `ws_agent.py`,
  ...
  cached terminal duplicate responses are also persisted before send so dropped
  duplicate-sends replay after reconnect.
...
- Agent-side Remote Assist starts from `pc_agent/ws_agent.py` handling the server command `remote_assist.request`; canonical user consent is now polled/decided through `pc_agent/ui_gui/main_window.py` and `/api/registry/agent/consents*` before the approved technical start command arrives. When the command includes `consent_status=approved`, the GUI starts the backend `/api/remote-assist/{session_id}/approve` technical step without showing a second consent dialog. `pc_agent/ui_bridge/event_bus.py` keeps unexpired `remote_assist_request` events replayable by `session_id` for late GUI/SSE subscribers, so a browser-approved Remote Assist command is not lost during GUI bridge reconnects.
...
- Windows stable agent `3.1.76` supersedes `3.1.75` for authenticated web-cabinet navigation and visible update progress: the main sidebar exposes `Веб кабинет`, `ws_agent.py` reports `update_request_state=applying` before update shutdown, and `main_window.py` shows `pending_restart` / `applying` / `restarting` as progress before the launcher restart. Windows stable agent `3.1.75` supersedes `3.1.74` for the web-cabinet GUI entrypoint: the account gate exposes `Веб кабинет`, opens the requester cabinet through the browser, and shows fallback links to the cabinet, login and password-reset request routes. Windows stable agent `3.1.74` supersedes `3.1.73` for stale self-update recovery: `orchestrator.py` archives a `pending_update.json` whose version is not newer than current `AGENT_VERSION` as `last_stale_pending_update.json`, then accepts the newer recommended build. Windows stable agent `3.1.68` supersedes `3.1.67` for the browser registration handoff and startup self-update refresh: web registration/login/profile setup preserve the original device-link `next` route, profile setup exposes the account-creation CTA, `ws_agent.py` requests the current server-recommended build once after successful startup handshake when no update is already pending, and server handshake can enqueue `agent_handshake_auto_update` for older installed agents that only know how to execute the `update` command. Windows stable agent `3.1.67` superseded `3.1.66` for the final strict browser-only registration cleanup: no normal-GUI local requester profile manager/sidebar, no `requester_profiles.json` writes from `ChatPanel`, no GUI profile registry sync, and no `profile.upsert` / `profile.select` automation actions. Windows stable agent `3.1.66` superseded `3.1.65` for strict browser-only web-first agent registration and localized Account page technical labels. Windows stable agent `3.1.65` superseded `3.1.64` as the priority Windows stable refresh from current HEAD. Windows stable agent `3.1.64` superseded `3.1.63` for terminal other-account request recovery: rejected, expired or canceled pending other-account login requests clear the local pending gate and fall back to current account-state. Windows stable agent `3.1.63` superseded `3.1.62` for the agent GUI browser-pairing handoff fix: non-local legacy `http://<host>:8666/api` settings open `/app/device/login` and `/app/device/register` on the HTTPS web boundary so secure web-session cookie confirmation can complete. Windows stable agent `3.1.62` superseded `3.1.61` for post-startup-hardening GUI/runtime fixes: unified user consent prompts, replay-safe Remote Assist request delivery to the GUI, account-gate recovery after revoked requester sessions, transient WSS/proxy retry behavior, command cancellation/restart recovery, account-session propagation through GUI automation, and service-catalog wizard automation hardening. Windows stable agent `3.1.61` superseded `3.1.60` for GUI startup failure handling: `ui_gui/__init__.py` keeps non-GUI API imports from loading Qt, `ws_agent.py` fails fast instead of falling back to `--no-gui` when Qt/PySide6 cannot load, and both launcher entrypoints stop after repeated immediate crashes if rollback is unavailable while writing `last_failed_launch.json` diagnostics.
