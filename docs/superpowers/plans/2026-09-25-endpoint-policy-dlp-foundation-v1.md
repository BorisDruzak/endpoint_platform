...
future 3.2.71 Agent; installed-Agent replay proof remains open after production migration. This is not
...
the feature remains gated to the future 3.2.71 release. USB interface
...
- [ ] Build the fixed `EndpointBrowserPolicy.exe` alone and prove a temporary, correctly configured Windows SCM service reaches `Running` and stops cleanly. Confirm the service identity and authenticated Agent-only channel; remove only that temporary service after the check. A PyInstaller import test alone does not pass this gate.
- [ ] Retain the retired 3.2.70 release as immutable failure evidence. Bump to the next Agent version, 3.2.71, rebuild the clean-source runtime and provenance manifest, signed MSI/Setup and immutable ZIP; verify source revision, every member hash, Authenticode validity, WSS startup proof and the exact rollback artifact before registering the new Setup release.
