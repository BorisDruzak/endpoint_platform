...
- [ ] Write failing tests for deterministic release metadata, stable extension ID, manifest/update XML digest, immutable registry, exact public download route, hash verification and no directory listing. Test that an unpublished or corrupt artifact cannot be served as a valid browser update.
...
- [ ] Build a signed CRX using an external protected key and verify its signature, embedded public key, pinned extension ID and packaged files before publishing immutable metadata separately from bytes. Check current official Chrome and Yandex enterprise-policy documentation against the installed versions, then generate Chrome `ExtensionSettings` and Yandex `ExtensionInstallForcelist`/`ExtensionSettings` templates without any global native-messaging blocklist.
...
The public extension ID is now pinned to a protected external signing key. Release tooling stages only approved MV3 source files, uses installed Chrome to sign the CRX, verifies the actual CRX3 signature and payload, and emits immutable versioned metadata and update XML from a clean committed source. The server now has migration `0031`, a registrar that verifies CRX3 identity and exact update URL, fixed digest-checked public routes, an admin-safe current-release projection and a publication runbook. The local `0.1.0` candidate passed registrar validation, but neither migration nor artifact publication has run on production. Browser policy applicator, native Bridge installation and Chrome/Yandex force-install acceptance remain open, so Task 6 stays unchecked. The browser installation proof belongs to Task 11, after the MSI and helper are packaged in Task 10.
