...
The public extension ID is now pinned to a protected external signing key. Release tooling stages only approved MV3 source files, uses installed Chrome to sign the CRX, verifies the actual CRX3 signature and payload, and emits immutable versioned metadata and update XML from a clean committed source. This is local build evidence only. Endpoint HTTPS artifact serving, release registry, browser policy applicator, native Bridge installation, Chrome/Yandex force-install acceptance and publication remain open, so Task 6 stays unchecked. The browser installation proof belongs to Task 11, after the MSI and helper are packaged in Task 10.
