...
5. **One CRX:** Chrome and Yandex officially support managed force-install from an update URL. A single signed CRX is the target; same-package acceptance is unproven. On 2026-09-25 the reference workstation has Chrome `153.0.8010.53` and Yandex Browser `26.8.3.1002` installed, so both must pass the managed-policy installation and native-messaging handshake with the identical CRX. Recheck both installed versions and effective enterprise-policy support at the live gate. In `agent_managed`, begin with the extension absent and prove Agent-driven force-install; in `external_managed`, Agent only reports observed state.
...
For status and acceptance, keep three observations separate: the Agent-owned machine policy value, the browser's effective enterprise policy, and the extension's Bridge heartbeat. A registry write does not prove that either browser accepted the policy. If the Agent cannot observe effectiveness, report it as unknown and require the browser's effective policy page at the Windows gate before claiming force-install. The current `BrowserFamilyStatusV1.installation_policy_state = APPLIED` describes the machine value, not browser acceptance; Task 8 must add a separate bounded effective-policy observation without changing the meaning of already emitted reports. Task 9 may show `Политика установки: применена` only after that observation confirms browser acceptance. Collect effective policy and download/handshake/heartbeat evidence independently in Task 11.
...
- [x] Build a signed CRX candidate using an external protected key; verify its signature, embedded public key, pinned extension ID, packaged files and immutable local metadata. This is a local candidate, not a published release.
- [ ] Recheck current official Chrome and Yandex enterprise-policy documentation against the installed versions and their effective policy pages. Generate Chrome `ExtensionSettings` and Yandex `ExtensionInstallForcelist`/`ExtensionSettings` templates without any global native-messaging blocklist; publish verified immutable metadata and bytes only after the release gate.
...
The public extension ID is now pinned to a protected external signing key. Release tooling stages only approved MV3 source files, uses installed Chrome to sign the CRX, verifies the actual CRX3 signature and payload, and emits immutable versioned metadata and update XML from a clean committed source. The server now has migration `0031`, a registrar that verifies CRX3 identity and exact update URL, fixed digest-checked public routes, an admin-safe current-release projection and a publication runbook. The local `0.1.0` candidate passed registrar validation, but neither migration nor artifact publication has run on production. The Windows registry applicator, authenticated fixed-ID helper service, typed Agent client, runtime dispatch and MSI source are now implemented with isolated tests; a PyInstaller helper build passed archive inspection. Installed-MSI proof, effective Chrome/Yandex policy, native Bridge handshake and force-install acceptance remain open, so Task 6 stays unchecked. The browser installation proof belongs to Task 11, after the MSI and helper are packaged in Task 10.
...
- [ ] Implement bounded best-effort USB and print watchers plus per-browser discovery/running, machine-policy owner/state, a separately represented effective-policy observation and Native Bridge health. Investigate a supported browser-side signal for effective enterprise policy; if none is reliable, report `UNKNOWN` rather than inferring effectiveness from the registry or heartbeat. Evolve the strict browser-status contract and server projection so `APPLIED` for a machine value cannot be interpreted as browser acceptance. Preserve compatibility with already emitted reports and old Agents. Coalesce bridge heartbeats into a typed WSS status envelope; persist latest per-family status and derive compliance on the server. Mark unsupported/unavailable honestly. Do not implement removable writes without proof of safe reliability.
...
- [ ] Write failing packaging tests for installed user sensor/bridge/policy helper, native-host registration for both browsers, exact origin, no global native-messaging blocklist, service-SID ACL, upgrade/ownership preservation and old-Agent rollback behavior; add ALT package tests only for supported paths. Verify the installed MSI's helper identity, binary location, authenticated Agent-to-helper channel and effective machine policy on a Windows test device.
