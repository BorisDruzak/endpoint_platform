...
- [ ] Add a focused fresh `external_managed` applicator test: start with an externally owned force-install entry and no Agent ownership marker; applying the policy must perform no installation-policy write and preserve foreign extensions/native hosts.
