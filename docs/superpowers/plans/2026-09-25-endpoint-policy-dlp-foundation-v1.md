...
The public extension ID is now pinned to a protected external signing key. Release tooling stages only approved MV3 source files, uses installed Chrome to sign the CRX, verifies the actual CRX3 signature and payload, and emits immutable versioned metadata and update XML from a clean committed source. The server now has migration `0031`, a registrar that verifies CRX3 identity and exact update URL, fixed digest-checked public routes, an admin-safe current-release projection and a publication runbook. The local `0.1.0` candidate passed registrar validation, but neither migration nor artifact publication has run on production. The Windows registry applicator core now has isolated tests for exact ownership, no-op repeat, foreign-entry preservation, conflict and safe relinquish. The authenticated helper, Agent dispatch, native Bridge installation and Chrome/Yandex force-install acceptance remain open, so Task 6 stays unchecked. The browser installation proof belongs to Task 11, after the MSI and helper are packaged in Task 10.
