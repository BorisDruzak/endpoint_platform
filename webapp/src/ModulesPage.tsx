...
type VersionDetail = { id: string; module_key: string; display_name: string; version: string; state: string; recipe: Recipe; validations: Validation[]; validations_total: number; labs: Lab[]; labs_total: number; passed_lab_platforms: string[] }
    ...
    <article className="capability-card" key={item.capability}>
      ...
      {item.parameters.length ? <ul>{item.parameters.map(parameter =>
        <li key={parameter.name}><code>{parameter.name}</code> · {parameterTypeLabels[parameter.value_type]} · {parameter.required ? 'обязательный' : 'необязательный'}
          ...
          {parameter.enum_values && ` · Значения: ${parameter.enum_values.join(', ')}`}
          {parameter.minimum != null && ` · Минимум: ${parameter.minimum}`}
...
export function ModulesPage() {
  ...
  const [historyLabOffset, setHistoryLabOffset] = useState(0)
  ...
  useEffect(() => {
    if (!detail) return
    let active = true
    request<{ data: VersionDetail }>(`/api/admin/console/modules/${encodeURIComponent(detail.module_key)}/versions/${encodeURIComponent(detail.version)}?validation_limit=20&validation_offset=${validationOffset}&lab_limit=20&lab_offset=${historyLabOffset}`)
      .then(value => { if (active) setDetail(value.data) })
      .catch(reason => { if (active) setError(message(reason)) })
    return () => { active = false }
  }, [revision, detail?.module_key, detail?.version, validationOffset, historyLabOffset])
  useEffect(() => { setValidationOffset(0); setHistoryLabOffset(0) }, [detail?.module_key, detail?.version])
  ...
  const labsReady = detail?.recipe.supported_platforms.every(platform =>
    detail.passed_lab_platforms.includes(platform)) ?? false
  return <>
    <div className="page-heading"><div><p className="eyebrow">РЕДАКТОР МОДУЛЕЙ</p><h1>Модули</h1><p className="muted">Последовательные рецепты из разрешённых возможностей Endpoint</p></div><button onClick={() => setRevision(current => current + 1)}>Обновить</button></div>
    ...
    {detail && <section className="panel"><button onClick={() => setDetail(null)}>Закрыть версию</button><h2>{detail.display_name} · {detail.version}</h2><p>{detail.module_key} · {stateLabels[detail.state] ?? detail.state}</p><p>Платформы: {detail.recipe.supported_platforms.join(', ')}</p><p>Подтверждённые платформы: {detail.passed_lab_platforms.length ? detail.passed_lab_platforms.map(platform => platformLabels[platform] ?? platform).join(', ') : 'Нет'}</p><ol>{detail.recipe.steps.map(step => <li key={step.step_id}>{step.step_id}: {step.capability} · {Object.entries(step.parameters).map(([name, binding]) => `${name}=${binding.kind === 'input' ? `input.${binding.name}` : String(binding.value)}`).join(', ')}</li>)}</ol><div className="actions"><button disabled={busy || !['draft', 'validation_failed'].includes(detail.state)} onClick={() => transition('validate', 'Проверка')}>Проверить модуль</button><button disabled={busy || detail.state !== 'validated' || !labsReady} onClick={() => transition('accept-labs', 'Принятие испытаний')}>Принять испытания</button><button disabled={busy || detail.state !== 'lab_accepted'} onClick={() => transition('publish', 'Публикация')}>Опубликовать</button><button disabled={busy || detail.state !== 'published'} onClick={() => transition('deprecate', 'Устаревание')}>Пометить устаревшим</button></div><section className="history-panel"><h3>Проверки</h3>
        {detail.validations.length ? <ul>{detail.validations.map((item, index) => <li key={index}>{dateText(item.completed_at)} · {item.status === 'succeeded' ? 'Успешно' : 'Ошибка'} · {item.validator_version}{item.error_codes.map(code => <span key={code}> · {errorLabels[code] ?? code} ({code})</span>)}{item.warning_codes.map(code => <span key={code}> · Предупреждение: {code}</span>)}</li>)}</ul> : <p>Проверок на этой странице нет.</p>}
        <div className="pagination"><button disabled={validationOffset === 0} onClick={() => setValidationOffset(Math.max(0, validationOffset - 20))}>Назад</button><span>{detail.validations_total ? `${validationOffset + 1}–${Math.min(validationOffset + 20, detail.validations_total)}` : '0'} из {detail.validations_total}</span><button disabled={validationOffset + 20 >= detail.validations_total} onClick={() => setValidationOffset(validationOffset + 20)}>Далее</button></div>
      </section>
      <section className="history-panel"><h3>Лабораторные испытания</h3>
        {detail.labs.length ? <ul>{detail.labs.map((item, index) => <li key={index}>{platformLabels[item.platform] ?? item.platform} · {labStatusLabels[item.status] ?? item.status} · {dateText(item.tested_at)} · <Link to={`/admin/operations?open=${item.operation_id}`}>Операция {item.operation_id}</Link></li>)}</ul> : <p>Испытаний на этой странице нет.</p>}
        <div className="pagination"><button disabled={historyLabOffset === 0} onClick={() => setHistoryLabOffset(Math.max(0, historyLabOffset - 20))}>Назад</button><span>{detail.labs_total ? `${historyLabOffset + 1}–${Math.min(historyLabOffset + 20, detail.labs_total)}` : '0'} из {detail.labs_total}</span><button disabled={historyLabOffset + 20 >= detail.labs_total} onClick={() => setHistoryLabOffset(historyLabOffset + 20)}>Далее</button></div>
      </section>
