...
export function UpdatesPage({ canWrite }: { canWrite: boolean }) {
  ...
  const [history, setHistory] = useState<Rollout[]>([])
  ...
  useEffect(() => {
    ...
    setHistoryError('')
    request<{ data: Rollout[]; total: number }>(`/api/admin/updates/rollouts?terminal=true&limit=50&offset=${historyOffset}`)
      .then(value => { if (active) { setHistory(value.data); setTotalHistory(value.total); setHistoryLoaded(true) } })
      .catch(reason => { if (active) { setHistoryError(reason instanceof Error ? reason.message : 'Историю загрузить не удалось'); setHistoryLoaded(true) } })
    return () => { active = false }
  }, [tab, historyOffset, revision])
  ...
  return <>
    ...
    {tab === 'history' && <section className="panel"><div className="table-top"><h2>История развёртываний</h2><span>{totalHistory} всего</span></div>{historyError ? <p className="error" role="alert">{historyError} <button onClick={() => setRevision(value => value + 1)}>Повторить</button></p> : !historyLoaded ? <p aria-live="polite">Загрузка истории…</p> : history.length ? <ul className="change-list">{history.map(item => <li key={item.id}><button onClick={() => setDetailId(item.id)}>{item.version} · {modes[item.mode] ?? item.mode}</button><span>{statuses[item.status] ?? item.status} · {dateText(item.completed_at ?? item.cancelled_at)}</span></li>)}</ul> : <p className="muted">Завершённых развёртываний пока нет.</p>}<div className="pagination"><button disabled={historyOffset === 0 || !historyLoaded} onClick={() => setHistoryOffset(Math.max(0, historyOffset - 50))}>Назад</button><span>{Math.min(historyOffset + 50, totalHistory)} из {totalHistory}</span><button disabled={historyOffset + 50 >= totalHistory || !historyLoaded} onClick={() => setHistoryOffset(historyOffset + 50)}>Далее</button></div></section>}
