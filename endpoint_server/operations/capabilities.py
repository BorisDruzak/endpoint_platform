...
def project_available_capabilities(
    ...
    if (
        connection is None
        or not network_primitives_enabled(settings)
        or connection.platform not in {"linux_amd64", "windows_amd64"}
        or not _has_minimum_network_primitive_version(connection.agent_version)
    ):
        return projected
