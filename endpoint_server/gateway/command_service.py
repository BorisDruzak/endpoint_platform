...
async def next_pending_command(session, device_id: UUID) -> GatewayCommandV1 | None:
...
class CommandService:
    ...
    async def deliver_next(
        ...
        async with self._session_provider() as session:
            try:
                payload = await next_pending_command(session, device_id)
                if payload is None:
        ...
        async with self._session_provider() as session:
            try:
                ...
                if stored is None:
                    ...
                    raise CommandStateRejected("result sequence conflicts with stored result")
...
__all__ = ["CommandService", "CommandStateRejected", "next_pending_command"]
